...
Method and coefficients: Fukushima (2020). See `src/wrightomega.jl`.
"""
...
@inline @fastmath function _wrightomega(x::Float64)
    # Branches are ordered so the arguments a device model actually produces
    # (roughly -50 < x < 300) are reached in two or three comparisons.
    if x < WO_X1                        # x < 0
        if x < WO_X0
...
wrightomega(x::Float32) = Float32(wrightomega(Float64(x)))
wrightomega(x::Real) = wrightomega(float(x))
wrightomega(x::Integer) = wrightomega(Float64(x))
